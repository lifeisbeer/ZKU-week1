...
template sudoku() {
    ...
    component rps[9*9*2]; // Declare enough RangeProof components to cover the entire puzzle and solution
    var pos;
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            pos = i*9+j; // Calculate the position of RangeProof component
            rps[pos] = RangeProof(4); // Instantiate a RangeProof component for each element of the puzzle
            rps[pos].in <== puzzle[i][j]; // Pass the sundoku number
            rps[pos].range[0] <== 0; // Pass the lower bound
            rps[pos].range[1] <== 9; // Pass the upper bound
            assert(rps[pos].out == 1); // Check that the RangeProof is true
            ...
            pos = 9*9+i*9+j; // Calculate the position of RangeProof component
            rps[pos] = RangeProof(4); // Instantiate a RangeProof component for each element of the solution, starting at index 9*9 onwards (to avoid overlap with puzzle)
            rps[pos].in <== solution[i][j]; // Pass the sundoku number
            rps[pos].range[0] <== 0; // Pass the lower bound
            rps[pos].range[1] <== 9; // Pass the upper bound
            assert(rps[pos].out == 1); // Check that the RangeProof is true
